...
/**
 * This class implements REPLACES THE Function-Types WITH LAMBDA
 *
...
abstract class MyGenericList[+A] {
  ...
  object GenericListTest extends App {
    ...
    println("----------------")
    println(singlyLinkedList12.map(element => element * 2).toString)

    println(singlyLinkedList13.filter((element: Int) => element % 3 == 0).toString)

    println("----------------")
    println(singlyLinkedList12 ++ singlyLinkedList13)
    println(singlyLinkedList13.flapMap((element: Int) => new ConsGeneric(element, new ConsGeneric(element+1, EmptyGenericList))).toString)
